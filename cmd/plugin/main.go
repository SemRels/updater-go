...
func run(stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if version == "" {
		_, _ = fmt.Fprintln(stderr, "updater-go: SEMREL_VERSION is required")
		return 1
	}
	...
	if getenv("SEMREL_DRY_RUN") == "true" {
		_, _ = fmt.Fprintf(stdout, "updater-go: [dry-run] would update %s to version %s\n", file, version)
		return 0
	}

	if err := plugin.NewUpdater().Update(file, varName, version); err != nil {
		_, _ = fmt.Fprintln(stderr, "updater-go:", err)
		return 1
	}

	_, _ = fmt.Fprintf(stdout, "updater-go: updated %s to version %s\n", file, version)
	return 0
}
